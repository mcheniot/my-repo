 ...
 * 
 */
public class MyClassicSiteHelperBK {
     ...
     */
    public MyClassicSiteHelperBK(String applicationName, String domain, String siteName) {
    ...
    private GoogleCredential getCredential() throws GeneralSecurityException, IOException {
    	File p12 = new File("c:/work/pkey.p12");
    ...
    public void getActivityFeed() throws IOException, ServiceException {
     ...
     // System.out.println(activityFeed.getHtmlLink());
      for (BaseActivityEntry<?> entry : activityFeed.getEntries()){
    	//entry.parseAtom(extProfile, input);
//        TextConstruct summary = entry.getSummary();
//        StringWriter sw = new StringWriter();
//        XmlWriter xw = new XmlWriter(sw);
//        summary.generateAtom(xw, "a");
        //System.out.println(xw.toString());
    	System.out.println(entry.getSummary().getPlainText() + " @ " + entry.getUpdated().toUiString() + "<br>") ;
    	//System.out.println(entry.getUpdated().toUiString() + "<br>");
        //System.out.println(entry.getSummary().getType());
        //XmlBlob div = new XmlBlob();
        //entry.getSummary().xhtml(div);
        
        //System.out.println("Href: " + entry.getSelfLink().getHref());
        
        XMLParserHelper xmlph = new XMLParserHelper();
        System.out.println("content url: <a href=\"" + xmlph.getContentUrl(entry.getSelfLink().getHref()) + "\">" + xmlph.getContentUrl(entry.getSelfLink().getHref()) + "</a>" + "<br>");
        System.out.println("\n<br>");
        
//        System.out.println("revisions link: " + entry.getRevisionLink().getHref());
//        System.out.println("revisions link: " + entry.getRevisionLink().getTitle());

        
      }
      Set<Category> entry = activityFeed.getCategories();
      Iterator<Category> itr = entry.iterator();
      while (itr.hasNext()){
    ...
    public BaseContentEntry<?> createPage(String kind, String title)
        throws MalformedURLException, MyClassicSiteException, IOException, ServiceException {
    ...
    public BaseContentEntry<?> createPage(String kind, String title, String parent)
        throws MyClassicSiteException, MalformedURLException, IOException, ServiceException {
      ...
      if (kind.equals("announcement")) {
        ...
        if (kind.equals("attachment") || kind.equals("webattachment")) {
          throw new MyClassicSiteException("Trying to create " + kind
                                   + ". Please use upload command instead.");
        } else {
          throw new MyClassicSiteException("Unknown kind '" + kind + "'");
        }
      }
